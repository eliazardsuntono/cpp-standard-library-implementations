...
/*
 * NOTE: there is a lot of repetitive code here - will work on fixing that and
 * implementing a clean portable solution (e.g in the ctors there are consistent
 * loops that could be cleaned up on initialization)
 */
...
public:
  ...
  explicit vector(size_t count, T const &val,
  ...
  vector &operator=(std::initializer_list<value_type> ilist) {
    try {
      ...
      std::cout << "Error in assignment with initializer_list: " << e.what()
                << std::endl;
      throw;
    }
    return *this;
  ...
  void shrink_to_fit() {
    try {
      ...
      capacity_ = size_;
                 ...
                 &e) { // NOTE: if allocation fails, then we just do nothing
      return;
    }
  }
